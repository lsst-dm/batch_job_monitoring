...
sample_interval = 15
# max run time in seconds. Sample every sample_interval seconds up to max_run.
max_run = 60
debug = False
...
stats = ["ClusterId", "ProcId", "JobStatus", "JobUniverse",
         "QDate", "ServerTime", "JobCurrentStartDate",
         "RemoteUserCpu", "EnteredCurrentStatus"]
extra_stats = ["Owner", "AccountingGroup", "CpusProvisioned",
               "NumRestarts", "NumJobStarts", "RemoteUserCpu",
               "RequestMemory", "RequestDisk", "RequestCpus",
               "ResidentSetSize_RAW", "DiskUsage_RAW", "HoldReason",
               "HoldReasonCode", "HoldReasonSubcode", "bps_job_name",
               "bps_job_label", "bps_campaign", "bps_job_quanta",
               "bps_job_summary", "bps_operator", "bps_payload",
               "bps_project", "bps_run", "bps_run_quanta"]
        ...
        for ijob in jobs:
            ...
            if jobjson['JobStatus'] == 1:
                ...
                try:
                    ...
                    # try to fix it by forcing an evaluation of
                    # the RequestMemory classad of the job
                    try:
...
# Now we have the json, let's push the record to the index,
# if we actually found anything

nsamples = max(nsamples, 1)

for totkey, totval in totals.items():
    # we are out of the while loop, so we have to divide the totals
